      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        {(Object.entries(groupedSchedules) as [string, Schedule[]][]).sort((a, b) => a[0].localeCompare(b[0])).map(([date, slots]) => (
          <section key={date} className="bg-white dark:bg-dark-surface rounded-4xl border border-gray-100 dark:border-white/5 shadow-soft overflow-hidden transition-colors">
            <div className="px-8 py-6 bg-navy flex justify-between items-center relative overflow-hidden">
               {/* Decorative Church Icon */}
               <span className="material-symbols-outlined absolute -right-4 -bottom-4 text-white/5 !text-8xl select-none">church</span>
              ...
              <div className="bg-white/10 px-4 py-2 rounded-2xl flex items-center gap-2 relative z-10">
                      ...
                      {booking ? (
                        <>
                          <div className="size-14 rounded-2xl bg-gray-200 dark:bg-white/10 flex items-center justify-center overflow-hidden border border-gray-300 dark:border-white/10 shrink-0">
                            {booking.photo_url ? (
                              <img src={booking.photo_url.startsWith('/') ? `http://localhost:3001${booking.photo_url}` : booking.photo_url} alt="Banda" className="w-full h-full object-cover" />
                            ) : (
